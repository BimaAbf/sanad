...
# One caregiver and four children with genuinely different histories, so the
# demo can be shown from a known state. Refuses in production.
seed-demo:
    uv --directory {{api}} run python -m app.cli seed-demo

# Drop the dev schema, re-apply every migration, reseed the curriculum and the
# demo family. The demo starts from here; see DEMO-RUNBOOK.md.
db-reset:
    uv run --no-project --python 3.12 tools/dev/reset_db.py

# THE gate. Rebuilds the database from the migrations, starts the API and the
# web app, runs every check the demo depends on, and prints SANAD DEMO READY or
# SANAD DEMO NOT READY with the exact failing gates. `--skip-e2e` for a fast
# inner loop; it can never print READY.
demo-check *ARGS:
    uv run --no-project --python 3.12 tools/dev/demo_check.py {{ARGS}}
