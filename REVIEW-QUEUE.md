...
## Added 2026-08-30 — the demo implementation pass

Everything below was authored by an agent and has been reviewed by nobody. Each
one is content or a threshold that reaches a caregiver or a child.

### #14 — the tracing reference paths · HANDWRITING / OT REVIEW

`services/api/seeds/tracing.py` holds a geometric approximation of eighteen
letter and numeral shapes, used as the guide a child traces and as the reference
the score is computed against. They are not transcribed from any Arabic
handwriting curriculum. The stroke ORDER and DIRECTION are not necessarily what
a child should be taught, and the diacritic dots are deliberately excluded from
the traced path (a child traces the body of ب; the dot is drawn for them).

A child can pass a tracing activity here by producing the right SHAPE. The file
makes no claim that the right shape is the right thing to teach first.

**Needed:** a handwriting or occupational-therapy review of every path, its
stroke order, and whether tracing-the-shape is the right task at all.

### #15 — the caregiver starting assessment · CLINICIAN + NATIVE-SPEAKER REVIEW

`services/api/app/modules/starting/domain/form.py` is a thirteen-question
instrument whose answers set a child's initial BKT priors and support profile.
Three things need a person:

* **the questions**, in Egyptian Arabic, and their concrete examples;
* **the derivation** — `BAND_PRIOR` (0.12/0.25/0.45/0.65), `BAND_SKILL_COUNT`
  (2/3/5/8 skills opened per area) and `BAND_STATE` are engineering choices with
  no evidence behind them. They decide where a child starts;
* **`shapes` maps to nothing.** The 88-skill curriculum has no shapes, so the
  question is asked, the answer is stored, and no prior is derived. Either the
  curriculum gains shapes or the question should go.

The one property that is enforced rather than reviewed: the form can never
produce `mastered`. The top band is 0.65 against a 0.90 threshold, and
`test_starting_form.py` asserts it.

### #16 — the drawing thresholds · OT REVIEW

`tutor/domain/drawing.py`: `PASS_THRESHOLD = 0.62`, `TOLERANCE = 0.10` of the
canvas, `MIN_DRAWN_LENGTH`, the length band and the smoothing window. They were
*measured* — swept over every reference glyph against seven negative and six
positive cases, with the highest negative at 0.590 and the lowest positive at
0.906 — but measured against synthetic tracings, not children's.

**Needed:** the same sweep against real tracings by children with Down syndrome,
and an OT's view on whether 0.10 of the canvas is the right tolerance for this
population's motor control.

### #17 — the mastery evidence rule, again · CLINICAL REVIEW

REVIEW-QUEUE #5's finding is fixed: the Hoeffding margin that made mastery
unreachable is gone, replaced by an anytime-valid mixture martingale (Ville's
inequality) plus a lifetime accuracy floor. The numbers a clinician now needs to
look at are:

* `ACCURACY_ALPHA = 1e-5` — the probability a guessing child is EVER called
  mastered, over a run of any length;
* `ALTERNATIVES = (0.60 … 0.95)` — what "a child who knows this" is assumed to
  score;
* `ACCURACY_FLOOR = 0.80` of everything they have ever been asked, unprompted;
* `MIN_SCORED_ATTEMPTS = 12`.

Together: twenty flawless independent two-choice attempts reach mastery, and a
child who is right 70 % of the time never does. Both of those are clinical
positions and neither has been reviewed.

### #18 — the new caregiver- and child-facing Arabic · NATIVE-SPEAKER REVIEW

Agent-written, unreviewed, and on screen:

* `apps/web/src/messages/ar-EG.json` — the `starting`, `inspector` and `session`
  sections;
* `tutor/domain/build.py` `INSTRUCTIONS` and `DEMONSTRATION_AR` — the line a
  child hears at the start of every activity;
* `tutor/domain/rewards.py` `ACHIEVEMENT_LABEL_AR`;
* `tutor/domain/report.py` `TEMPLATE_AR` — the caregiver's session summary.
