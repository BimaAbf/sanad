...
================================================================================
THIS IS THE CAREGIVER-REPORTED PATH, NOT THE ONE THE CHILD APP USES
================================================================================
Everything here takes a `result` the CALLER declares. That was the original
child-app contract and it is no longer one: the child surface runs the tutor
loop (`app/modules/tutor/`), where the server evaluates the response against an
answer key the client is never sent, and it is the only path by which SANAD
itself decides whether a child was right.

What remains here is a caregiver saying what happened — the same kind of claim
`attempt_result.caregiver_confirmed` already exists for, and one a caregiver is
entitled to make about their own child. It is scoped by `assert_child_access`
like everything else, so it can only ever be a claim about a family's OWN
child, and no client in this repository calls it.

If you are looking for the path the demo runs on, it is `POST
/tutor/sessions/{id}/respond`. If you are adding a route here, ask first
whether the tutor loop should own it instead.
================================================================================
