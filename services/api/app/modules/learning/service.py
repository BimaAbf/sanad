...
class MasteryService:
    ...
    async def apply_session(
        ...
        *,
        ...
        `only` restricts the recompute to ONE (skill, modality), and the tutor
        loop always passes it. Without it, calling this after every response
        re-evaluates every skill the session has touched so far -- and because
        `next_state` advances one rung per evaluation, a child answering their
        fifth activity would silently push the first four skills up a rung each
        on the strength of no new evidence at all. With it, a skill moves when
        that skill was practised, which is the only defensible reading of "one
        evaluation per opportunity".
        """
        targets = await self._repo.session_targets(session_id)
        if only is not None:
            targets = [target for target in targets if target == only]
        if not targets:
            return []
        ...
        for skill_id, modality in {*targets, *stored}:
            ...
            state.progress[(str(skill_id), modality)] = SkillProgress(
                # The fold restarts from the PRIOR, not from the stored
                # `p_known` -- that is what makes it idempotent. The prior is
                # this child's own when a starting assessment set one, and the
                # population default otherwise. It is written once and never
                # updated, so it is a constant of the fold rather than a value
                # the fold's own output could feed back into.
                bkt=BktState(p_known=_prior(row)),
...
def _prior(row: Any | None) -> float:
    """This child's starting estimate for one skill.

    `bkt.P_L0` is a population default chosen because colours and body parts are
    already familiar from home life. When a caregiver has told us where THIS
    child is, that is better evidence than a population default, and ignoring it
    would mean the assessment changed nothing the child experiences.
    """
    if row is None or getattr(row, "p_prior", None) is None:
        return P_L0
    return clamp(float(row.p_prior), P_MIN, P_MAX)
