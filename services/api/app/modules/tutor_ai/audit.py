...
class TutorAuditRepository:
    ...
    async def record_decision(
        ...
        *,
        ...
        # `str()` first: asyncpg returns its own `pgproto.UUID`, and
        # `uuid.UUID(that)` raises AttributeError rather than converting. This
        # module had no caller until the tutor loop, so the defect had never
        # been executed.
