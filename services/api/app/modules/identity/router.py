...
async def read_me(
    ...
    links = await service.repo.list_links(caregiver_id)
    return MeResponse(
        id=caregiver.id,
        ...
        children=[
            # display_name is filled by the children module once it exists; the
            # link itself is authoritative here.
            CaregiverChildLink(id=link.child_id, display_name="", role=str(link.role))
            for link in links
        ],
    )
