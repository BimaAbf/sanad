...
async def test_export_and_erasure_refuse_rather_than_returning_a_job_that_will_never_run() -> None:
    """A 202 for a job nothing will pick up is a lie about a data-subject request.

    Both routes used to mint a job id and return 202 ACCEPTED. There is no
    worker pool, so nothing would ever produce the export or perform the
    erasure — the caregiver was told their request had been accepted and it had
    not been. These now refuse with the reason, and will keep refusing until
    `export_child_data` and `erase_child` have a runner. Archiving, which is
    real, is unaffected and is asserted elsewhere.
    """
    from app.core.errors import ServiceUnavailable
    from app.modules.children.router import delete_child, export_child

    child_id = uuid.uuid4()

    with pytest.raises(ServiceUnavailable):
        await export_child(child_id, _access=None)  # type: ignore[arg-type]

    with pytest.raises(ServiceUnavailable):
        await delete_child(
            child_id,
            _access=None,  # type: ignore[arg-type]
