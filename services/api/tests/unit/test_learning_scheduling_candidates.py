...
def test_the_evidence_martingale_starts_at_one_and_ignores_shown_answers() -> None:
    """log M = 0 means "exactly as likely under guessing as under learning"."""
    assert evidence_log_ratio([]) == 0.0
    shown = [_attempt(True, PromptLevel.FULL_MODEL) for _ in range(50)]
    assert evidence_log_ratio(shown) == 0.0, (
        "being shown the answer fifty times must carry no evidence at all"
    )


def test_the_evidence_martingale_rises_on_success_and_falls_on_failure() -> None:
    correct = [_attempt(True) for _ in range(10)]
    wrong = [_attempt(False) for _ in range(10)]
    assert evidence_log_ratio(correct) > 0.0
    assert evidence_log_ratio(wrong) < 0.0
    # Monotone in the number of successes, which is what makes "more evidence"
    # mean more evidence.
    assert evidence_log_ratio(correct) > evidence_log_ratio(correct[:5]) > 0.0


def test_a_correct_answer_out_of_four_is_worth_more_than_out_of_two() -> None:
    """The martingale prices each attempt at ITS OWN chance level."""
    two = evidence_log_ratio([_attempt(True, n=2) for _ in range(5)])
    four = evidence_log_ratio([_attempt(True, n=4) for _ in range(5)])
    assert four > two


def test_twenty_flawless_independent_two_choice_attempts_clear_the_threshold() -> None:
    """The defect this replaced: at n=20 the old rule demanded accuracy 1.102.

    This is requirement (B) stated as arithmetic — genuine repeated independent
    success reaches the bar after a bounded amount of evidence — and it is the
    exact boundary, so a change that makes mastery harder fails here loudly.
    """
    assert evidence_log_ratio([_attempt(True) for _ in range(20)]) >= EVIDENCE_LOG_THRESHOLD
    assert evidence_log_ratio([_attempt(True) for _ in range(19)]) < EVIDENCE_LOG_THRESHOLD
...
def test_evaluate_names_every_unmet_condition() -> None:
    ...
    assert set(decision.unmet) == {
        ...
        "independent_ratio",
        "min_scored_attempts",
        # An empty history fails the evidence guard and the accuracy floor
        # too, and says so. The earlier rule reported only the attempt-count
        # failure, which read as "nearly there" for a child with no history.
