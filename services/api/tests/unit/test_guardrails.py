...
def test_every_decision_point_in_the_enum_is_registered() -> None:
    ...
    The chat surfaces were added with the `ai_decision_point` values in
    migration 0009. `caregiver_chat` produces prose a parent reads;
    ...
    `tutor_brain` was added with migration 0014. It is the per-activity teaching
    decision: it chooses from a candidate set the deterministic engine built,
    and every field it returns is a closed set, so it carries both of those
    layers and neither of the prose ones -- it writes no prose at all.
    """
