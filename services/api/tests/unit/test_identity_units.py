...
# --- the local SMS provider ------------------------------------------------


async def test_the_null_sms_provider_prints_only_the_ascii_code() -> None:
    """Two defects this one line has had, both fixed here.

    It used to print the whole Arabic message, which raised
    `UnicodeEncodeError` on a Windows cp1252 console INSIDE the request — so
    every sign-in on Windows was a 500 for a message that had in fact been
    sent. Then it filtered on `str.isdigit()`, which is true for Arabic-Indic
    digits, so the printed code picked up the ٥ from "صالح ٥ دقايق" and was one
    character longer than the code the caregiver had to type.
    """
    import io
    import sys

    from app.modules.identity.service import OTP_MESSAGE_AR
    ...
    try:
        assert await provider.send(
            phone_e164="+201000000000", message=OTP_MESSAGE_AR.format(code="123456")
        )
    finally:
        sys.stdout = original

    printed = captured.getvalue().strip()
    assert printed == "[NullSms] -> +201000000000 code=123456"
    assert printed.isascii(), "an Arabic byte here is a 500 on a cp1252 console"
    # The provider still records the full message: the tests that read the code
    # back read it from there, not from stdout.
    assert provider.sent == [("+201000000000", OTP_MESSAGE_AR.format(code="123456"))]
