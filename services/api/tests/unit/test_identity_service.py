...
class FakeRepo:
    ...
    #: child_id -> display_name, for the `/me` join. The real repository reads
    #: it from `children`; this fake has no children table.
    child_names: dict[uuid.UUID, str] = field(default_factory=dict)
    ...
    async def list_links_with_names(
        self, caregiver_id: uuid.UUID
    ) -> list[tuple[uuid.UUID, str, str]]:
        """The join `/me` reads. A name here rather than the empty string the
        response used to carry for every child."""
        return [
            (link.child_id, self.child_names.get(link.child_id, ""), link.role)
            for key, link in self.links.items()
            if key[0] == caregiver_id
        ]
