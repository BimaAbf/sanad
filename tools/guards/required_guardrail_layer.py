...
STATUS: PASS, over eleven decision points. The registry it reads is
        `app/guardrails/chain.py::REQUIRED_LAYERS`; the newest entry is
        `tutor_brain`, the per-activity teaching decision, which carries
        CandidateSetLayer (it selects from a set the deterministic engine built)
        and ClosedEnumLayer (every field it returns is a closed set).
"""
...
REQUIRED_LAYERS: dict[str, frozenset[str]] = {
    ...
    "tutor_brain": frozenset({"CandidateSetLayer", "ClosedEnumLayer"}),
...
def check() -> GuardResult:
    ...
    for decision_point, required in REQUIRED_LAYERS.items():
        if decision_point not in declared:
            result.violation(sources[0], 1, f"decision point {decision_point!r} is not registered")
            continue
