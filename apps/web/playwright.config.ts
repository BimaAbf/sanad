...
export default defineConfig({
  ...
  projects: [
    {
      // One sign-in, shared. `RATE_OTP_PER_PHONE_PER_HOUR` is three, and the
      // critical path needs a signed-in caregiver on five screens.
      name: "demo-setup",
      testMatch: /demo\.setup/,
      use: { ...devices["Pixel 5"] },
    },
    {
      // The critical path. Named separately so `just demo-check` can run this
      // one alone: it is the gate, and the other two projects are quality
      // checks that must not be able to hold the demo hostage.
      name: "demo",
      testMatch: /demo-critical-path/,
      fullyParallel: false,
      dependencies: ["demo-setup"],
      use: { ...devices["Pixel 5"], storageState: "e2e/.auth/caregiver.json" },
    },
    {
      name: "caregiver",
