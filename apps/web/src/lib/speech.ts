...
export interface ListenHandlers {
  /**
   * `confidence` is what the browser reported, or 0 when it reported nothing.
   *
   * Zero is passed through as zero rather than substituted with a guess: the
   * server distinguishes "the recogniser was unsure" from "the recogniser does
   * not estimate confidence", and inventing a number here would collapse the
   * two and make every attempt on Firefox or Safari read as uncertain.
   */
...
export function listenOnce(handlers: ListenHandlers): (() => void) | null {
  ...
  recognition.onresult = (event) => {
    ...
    if (text.trim()) handlers.onResult(text.trim(), best?.confidence ?? 0);
  };
