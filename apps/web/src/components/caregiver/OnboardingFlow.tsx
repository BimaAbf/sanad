...
/**
 * Onboarding: phone → OTP → child → consent → the starting assessment.
 *
 * Signed in already? The flow opens on the child step. It is the same flow —
 * adding a second child is the same three screens minus the two that
...
export function OnboardingFlow({ signedIn }: { signedIn: boolean }) {
  ...
  // A caregiver who is already signed in starts at the child step. Without
  // this, adding a second child means verifying a code again — and the OTP is
  // rate limited at three an hour, so "add another child" could simply refuse.
  const [step, setStep] = useState<Step>(signedIn ? "child" : "phone");
