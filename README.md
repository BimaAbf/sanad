...
**To run the demo:** [`DEMO-RUNBOOK.md`](DEMO-RUNBOOK.md). What is built and
what was tested: [`DEMO-IMPLEMENTATION-STATUS.md`](DEMO-IMPLEMENTATION-STATUS.md).
One command answers whether it is ready:
...
```
just demo-check  # THE gate: is the demo ready, yes or no
just test        # the CI gate, verbatim
just test-quick  # the same minus the tests that need a live Postgres
just lint
just guards
```

`demo-check` rebuilds the database from the migrations, seeds the demo family,
starts both processes, and runs every gate including the critical path in a real
browser. It prints `SANAD DEMO READY` or `SANAD DEMO NOT READY` and the exact
failing gates. `--skip-e2e` gives a fast inner loop and can never print READY.
